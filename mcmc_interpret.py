...
MAX_DATA_SITES		=	10				# If this value is changed, change the "#define MAX_DATA_SITES 20" within the C code	
...
chain_params = str(file_id)+"output/mcmc_parameter_chains_"+str(file_id[7:24])+".txt"

# TEMP FOR TESTING
#chain_params = 'output/mcmc_parameter_chains_MOA_2011_BLG_197.txt'
####################
...
line_number = -2
chain_parameters  = csv.reader(open(chain_params,'r'), delimiter=',', skipinitialspace=True)
for line in chain_parameters:
	if line_number == -2:
		nothing = 0
	elif line_number == -1:
		nwalkers = int(float(line[0]))
		sample = int(float(line[2]))
		
		d_arr	= np.zeros([nwalkers*sample],np.float64)
		q_arr	= np.zeros([nwalkers*sample],np.float64)
		rho_arr	= np.zeros([nwalkers*sample],np.float64)
		u0_arr	= np.zeros([nwalkers*sample],np.float64)
		phi_arr	= np.zeros([nwalkers*sample],np.float64)
		t0_arr	= np.zeros([nwalkers*sample],np.float64)
		tE_arr	= np.zeros([nwalkers*sample],np.float64)
	elif line_number == 0:
		for i in xrange(0,nwalkers*sample):
			d_arr[i]		= np.float64(line[i])
	elif line_number == 1:
		for i in xrange(0,nwalkers*sample):
			q_arr[i]		= np.float64(line[i])
	elif line_number == 2:
		for i in xrange(0,nwalkers*sample):
			rho_arr[i]	=np.float64(line[i])
	elif line_number == 3:
		for i in xrange(0,nwalkers*sample):
			u0_arr[i]	=np.float64(line[i])
	elif line_number == 4:
		for i in xrange(0,nwalkers*sample):
			phi_arr[i]	= np.float64(line[i])
	elif line_number == 5:
		for i in xrange(0,nwalkers*sample):
			t0_arr[i]	= np.float64(line[i])
	elif line_number == 6:
		for i in xrange(0,nwalkers*sample):
			tE_arr[i]	= np.float64(line[i])
			
	line_number += 1

with open(str(file_id)+"output/emcee_results.txt", "w") as f:
	f.write('%s\nemcee results output file (parameter, mean, std dev)\n'% file_id[7:24])

x_lim_l = np.zeros([7],np.float32)
x_lim_u = np.zeros([7],np.float32)
y_lim_l = np.zeros([7],np.float32)
y_lim_u = np.zeros([7],np.float32)

parameter = [d,q,rho,u0,phi,t0,tE]
parameter_arr = [d_arr,q_arr,rho_arr,u0_arr,phi_arr,t0_arr,tE_arr]
parameter_name = ['d','q','rho','u0','phi','t0','tE']

plt.clf()
fig = plt.figure(figsize=(51.2,51.2))

for i in range(0,7):
	
	ax = plt.subplot(7,7,i*7+i+1)
	plt.hist(parameter_arr[i], 500, facecolor='blue', edgecolor='blue')
	title = 'MCMC Histogram of '+parameter_name[i]+'\n'+str(file_id[7:-1])																																											# Labels for the figures
	xlabel = parameter_name[i]
	ylabel = ''
	#plt.title(title,fontsize='small')
	#ax.set_xticklabels([])
	#ax.set_yticklabels([])
	#plt.savefig('output/mcmc_hist_of_'+parameter_name[j]+'.png')
	
	parameter[i] = np.mean(parameter_arr[i])
	std_dev = np.std(parameter_arr[i])
	
	print 'Parameter '+parameter_name[i]+': Mean = '+str(parameter[i])+',	Std Dev = '+str(std_dev)
	with open(str(file_id)+"output/emcee_results.txt", "a") as f:																																																							# Write emcee mean and SD values to file
		f.write('%s, %0.17f, %0.17f \n'% (parameter_name[i], parameter[i], std_dev))
	
	for j in range(0,7):
		locs,labels = plt.xticks()
		plt.xticks(locs, map(lambda x: "%g" % x, locs))
		locs,labels = plt.yticks()
		plt.yticks(locs, map(lambda y: "%g" % y, locs))

		if i==j:
			continue
		ax = plt.subplot(7,7,i+j*7+1)
		plt.scatter(parameter_arr[i], parameter_arr[j], color='k',marker='x', s = 1, c=np.random.rand(nwalkers*sample),cmap='jet')
		title = 'MCMC Histogram of '+parameter_name[i]+'/'+parameter_name[j]+'\n'+str(file_id[7:-1])																																											# Labels for the figures
		
		x_low, x_up = ax.get_xlim()
		y_low, y_up = ax.get_ylim()
		
		if x_low < x_lim_l[i] or x_lim_l[i] == 0:
			x_lim_l[i] = x_low
		if x_up > x_lim_u[i] or x_lim_u[i] == 0:
			x_lim_u[i] = x_up
		
		if y_low < y_lim_l[j] or y_lim_l[j] == 0:
			y_lim_l[j] = y_low
		if y_up > y_lim_u[j] or y_lim_u[j] == 0:
			y_lim_u[j] = y_up
...
for i in range(0,7):
	for j in range(0,7):
		ax = plt.subplot(7,7,i+j*7+1)
		ax.set_xlim(x_lim_l[i],x_lim_u[i])
		...
		ax = plt.subplot(7,7,i+j*7+1)
		ax.set_ylim(y_lim_l[j],y_lim_u[j])
		
		
for i in range(0,7):
	ax = plt.subplot(7,7,i+1)
	plt.title(parameter_name[i],size=64)
	
	ax = plt.subplot(7,7,i+(6*7)+1)
	ax.tick_params(axis='x', which='both', labelsize='medium')
	plt.xticks(rotation='vertical')
	
	ax = plt.subplot(7,7,i*7+1)
	plt.ylabel(parameter_name[i], size=64)
	ax.tick_params(axis='y', which='both', labelsize='medium')
	



ax2 = plt.subplot(7,7,2)
ax2.tick_params(axis='y', which='both', labelsize='medium', pad=423)
...
plt.savefig(str(file_id)+'output/full_emcee_output.png')
...
####----- store convolved mag map on the texture memory
...
u0n = u0 - np.sin(phi)*b
t0n = t0 - tE*(u0/np.tan(phi) - u0n/np.tan(phi))
...
tbot = t0n-10*tE
u1bot = ((tbot - t0n)/tE)*np.cos(phi) - u0n*np.sin(phi)
u2bot = ((tbot - t0n)/tE)*np.sin(phi) + u0n*np.cos(phi)

ttop = t0n+10*tE
u1top = ((ttop - t0n)/tE)*np.cos(phi) - u0n*np.sin(phi)
u2top = ((ttop - t0n)/tE)*np.sin(phi) + u0n*np.cos(phi)
xbot = [u1bot,u1top]
ybot = [u2bot,u2top]
...
title = 'Convolved (rho='+str(rho)+') magnification map of best fitted binary model parameters\n'+str(file_id[7:24])
xlabel = 'u1'
ylabel = 'u2'
plt.title(title,fontsize='small')
plt.xlabel(xlabel,fontsize='small')
plt.ylabel(ylabel,fontsize='small')
plt.savefig(str(file_id)+'output/emcee_best_convolved_mag_map.png')
...
for i in range(len(data_types)):
	if i == 0:
		...
		if data_types[i] == 0:											# MOA
			...
			gridshape_chi2 = (1,1)
			single_chi2_calc(np.float64(t0), np.float64(tE), np.float64(u0), np.float64(phi), np.float64(b), np.float64(map_limits), np.int32(num_points), np.float32(x0), np.float32(x0+dx*num_points-dx), drv.In(DATA_ROWS[0]), drv.In(m0[0]), drv.In(ts[0:DATA_ROWS[0]]), drv.In(mags[0:DATA_ROWS[0]]), drv.In(sigs[0:DATA_ROWS[0]]), np.int32(data_sets), drv.In(data_types[0]), drv.Out(Fo), drv.Out(K), drv.Out(chi2), texrefs=[texref], block=blockshape_chi2, grid=gridshape_chi2)
			...
			u1, u2, tau = rd_zeta(u0n, phi, new_ts, t0n, tE, map_limits)																																											# calc the u1,u2 coordinates on the mag map for the time steps (ts)
			...
			#u1, u2, tau = rd_zeta(u0n, phi, ts, t0n, tE, map_limits)																																											# calc the u1,u2 coordinates on the mag map for the time steps (ts)
			...
			plt.xlabel(xlabel,fontsize='small')
			plt.ylabel(ylabel,fontsize='small')
			plt.savefig(str(file_id)+'output/emcee_MOA_best_parameters_LC.png')
			
			with open(str(file_id)+"output/emcee_results.txt", "a") as f:																																																							# Write emcee mean and SD values to file
				f.write('Fo, %0.17f, 0.0 \n'% (Fo))
			...
			A_interp = m0[i] - 2.5*np.log10((A_interp-1)*K+1)																																																											# Calc the flux of these mag values from the solved Fo value outputed from the CUDA func.
			
			plt.plot(new_ts,A_interp,'b-')
			
			#u1, u2, tau = rd_zeta(u0n, phi, ts[0:DATA_ROWS[0]], t0n, tE, map_limits)
			...
			plt.xlabel(xlabel,fontsize='small')
			plt.ylabel(ylabel,fontsize='small')
			plt.savefig(str(file_id)+'output/emcee_OGLE_best_parameters_LC.png')
		...
		if data_types[i] == 0:											# MOA
			...
			blockshape_chi2 = (int(blockDim_x), 1, 1)
			gridshape_chi2 = (1,1)
			single_chi2_calc(np.float64(t0), np.float64(tE), np.float64(u0), np.float64(phi), np.float64(b), np.float64(map_limits), np.int32(num_points), np.float32(x0), np.float32(x0+dx*num_points-dx), drv.In(DATA_ROWS[i]), drv.In(m0[i]), drv.In(ts[DATA_ROWS[i-1]:np.sum(DATA_ROWS[0:i+1])]), drv.In(mags[DATA_ROWS[i-1]:np.sum(DATA_ROWS[0:i+1])]), drv.In(sigs[DATA_ROWS[i-1]:np.sum(DATA_ROWS[0:i+1])]), np.int32(data_sets), drv.In(data_types[i]), drv.Out(Fo), drv.Out(K), drv.Out(chi2), texrefs=[texref], block=blockshape_chi2, grid=gridshape_chi2)
			...
			#u1, u2, tau = rd_zeta(u0n, phi, ts[DATA_ROWS[i-1]:np.sum(DATA_ROWS[0:i+1])], t0n, tE, map_limits)																																											# calc the u1,u2 coordinates on the mag map for the time steps (ts)
			...
			#u1, u2, tau = rd_zeta(u0n, phi, ts[DATA_ROWS[i-1]:np.sum(DATA_ROWS[0:i+1])], t0n, tE, map_limits)
