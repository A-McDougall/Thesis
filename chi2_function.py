...
MAX_THREADS				=	np.int32(512)
MAX_DATA_SITES			=	np.int32(10)
...
def rd_zeta(u0, phi, ts, t0, tE, map_limits):																																																																							# produce the (x,y) coordinates of real data (moa_ts)	
	tau = ts
	#zeta		= np.zeros([t],np.complex64)
...
//////////////////////////////////////////////////////////////	No Basline Magnitude (NBM) data Chi^2 calculation 
	__device__ double nbm_chi2_func(double *variables, double *u0, double *phi, double *b, double *map_limit, int *num_points, float *dim_min, float *dim_max, int *TIME_STEPS, int *pow_2_n, double *m0, double *Fo, double *ts, float *mags, float *sigs)
	{
		__shared__ double ynew[MAX_DATA_POINTS];
		__shared__ float K_0, K_1;
		__shared__ float interp_mag[MAX_DATA_POINTS], a_lr[MAX_DATA_POINTS],b_lr[MAX_DATA_POINTS],d_lr[MAX_DATA_POINTS],e_lr[MAX_DATA_POINTS],f_lr[MAX_DATA_POINTS];
		__shared__ int power_limit;
		float pixel_x, pixel_y;
		double u0n, u1, u2, u, usq, t0n, t0, tE;
		float m0_l = *m0;
		...
		while (index < *TIME_STEPS)																																																																			// Set while for cases where more data points then max threads allowed
		{
			...
			a_lr[index]				=	1	/	(logf(10)*pow(10.0,(-0.4*(mags[index] - m0_l)))*sigs[index]*logf(10)*pow(10.0,(-0.4*(mags[index] - m0_l)))*sigs[index]);
			b_lr[index]				=	interp_mag[index]	/	(logf(10)*pow(10.0,(-0.4*(mags[index] - m0_l)))*sigs[index]*logf(10)*pow(10.0,(-0.4*(mags[index] - m0_l)))*sigs[index]);
			d_lr[index]				=	(interp_mag[index]*interp_mag[index])	/	(logf(10)*pow(10.0,(-0.4*(mags[index] - m0_l)))*sigs[index]*logf(10)*pow(10.0,(-0.4*(mags[index] - m0_l)))*sigs[index]);
			
			e_lr[index]				=	pow(10.0,(-0.4*(mags[index] - m0_l)))	/	(logf(10)*pow(10.0,(-0.4*(mags[index] - m0_l)))*sigs[index]*logf(10)*pow(10.0,(-0.4*(mags[index] - m0_l)))*sigs[index]);
			f_lr[index]				=	(pow(10.0,(-0.4*(mags[index] - m0_l)))*interp_mag[index])	/	(logf(10)*pow(10.0,(-0.4*(mags[index] - m0_l)))*sigs[index]*logf(10)*pow(10.0,(-0.4*(mags[index] - m0_l)))*sigs[index]);
		...
		{																																																																																		// this is important for the parrallel summing techniques that require the arrays to be 2^n in size.
			interp_mag[index] = 0;
			a_lr[index] = b_lr[index] = d_lr[index] = e_lr[index] = f_lr[index] = 0;
			
			index += thread_max;
		}
//
//		Solve using basis functions, as part of a linear regression technique.
//		
//		alpha =	[	a	b	]		beta =	[	e	]
//				[	c	d	]				[	f	]
//
//		C	=	inv(a)	=	_1_	*	[	d	-b	]	=	[	d / (ad-bc)		-b/(ad-bc)	]
//							ad-bc	[	-c	a	]		[	-c/(ad-bc)		a/(ad-bc)	]
//
//
//		A	=	C * beta	=	[	d / (ad-bc)		-b/(ad-bc)	]	*	[	e	]	=	[	(de) / (ad-bc)	+	(-bf)/(ad-bc)	]
//								[	-c/(ad-bc)		a/(ad-bc)	]		[	f	]		[	(-ce)/(ad-bc)	+	(af)/(ad-bc)	]
//		
//		solutions = A[0], A[1]
//		
//		error = sqrt(diag(C))		=	[	(d / (ad-bc))^0.5	]
//										[	(a / (ad-bc))^0.5	]
//		--------------------
//		To solve for values of K in:		y	=	A_i*K_1	+	K_0		
//
//		where	a	=	sum(	1 		/ sigs^2	)
//				b	=	sum(	A_i 	/ sigs^2	)
//				c	=	sum(	A_i		/ sigs^2	)			note that c == b
//				d	=	sum(	A_i^2	/ sigs^2	)
//		
//				e	=	sum(	y		/	sigs^2	)
//				f	=	sum(	y*A-i	/	sigs^2	)
//		
//																																																																																				// Utilising the array size of 2^n, it is possible to do a parallel sumation, by computing half the array dimms and summing that value with its index + half the arrays size.
//																																																																																				// then repeat this procedure with half again, and loop until all values are summed down to a single element.
//																																																																																				// This method has been taken from the CUDA SDK.		
		k = power_limit / 2;
		__syncthreads();
		while (k!=0)																																																																													// Calculate the summed Fo contributing values
		{
			if (threadIdx.x < k)
			{
				a_lr[threadIdx.x] += a_lr[threadIdx.x + k];
				b_lr[threadIdx.x] += b_lr[threadIdx.x + k];
				d_lr[threadIdx.x] += d_lr[threadIdx.x + k];
				
				e_lr[threadIdx.x] += e_lr[threadIdx.x + k];
				f_lr[threadIdx.x] += f_lr[threadIdx.x + k];
			}
			__syncthreads();
			k /= 2;
		}
		...
		if (threadIdx.x == 0)
		{
			K_0	=	(d_lr[0]*e_lr[0]) / (a_lr[0]*d_lr[0]-b_lr[0]*b_lr[0])	+	(-b_lr[0]*f_lr[0])/(a_lr[0]*d_lr[0]-b_lr[0]*b_lr[0]);
			K_1	=	(-b_lr[0]*e_lr[0])/(a_lr[0]*d_lr[0]-b_lr[0]*b_lr[0])	+	(a_lr[0]*f_lr[0])/(a_lr[0]*d_lr[0]-b_lr[0]*b_lr[0]);
		}
		...
		while (index < *TIME_STEPS)																																																																			// Calculate the squared difference between the model value and the real data
		{
			ynew[index]	=	pow((mags[index] - m0_l + 2.5*log10(interp_mag[index]*K_1 + K_0)),2.0)	/	(sigs[index]*sigs[index]);
			index += thread_max;
		}
		while (index < power_limit)
		{
			ynew[index] = 0;																																																																									// Ensure the remaing part of the 2^n array are filled with zeros, to prevent errors occuring when summed
			index += thread_max;
		}
		...
		{
//			*chi2	= ynew[0];
			*Fo		= K_1+K_0;																																																												// Store the source flux parameter at its associated pointer so it can be used outside the function
		}
...
//////////////////////////////////////////////////////////////	Chi^2 type controller
	__device__ double chi2_controller(double *variables, double *u0, double *phi, double *b, double *map_limit, int *num_points, float *dim_min, float *dim_max, int *TIME_STEPS, int *pow_2_n, double *Fo, double *K, double *ts, float *mags, float *sigs, int *data_sets, double *m0, int *data_types)
	{
		__shared__ double chi2;
		__shared__ int start;
		double  ytry, Fotry, Ktry;
		int i;
...
//																																																																																				// Modified a lot to hard code in a lot of unessary variables, and remove NR specific functions which defined arrays.
	__device__ double amotry(double p[], double y[], double psum[], int ihi, double fac,
		double *u0, double *phi, double *b, double *map_limit, int *num_points, float *dim_min, float *dim_max, int *TIME_STEPS, int *pow_2_n, double *Fo, double *K, double *ts, float *mags, float *sigs, int *data_sets, double *m0, int *data_types)
	{
	...
	//	ptry=vector(1,NDIM);																																																																								// Replaced NR array initialising method with standard C practice, ensure the array size is +1 what it needs to be, due to NR conversion of code from Fortran where array indexing starts at 1 instead of 0
		...
		ytry=chi2_controller(ptry, u0, phi, b, map_limit, num_points, dim_min, dim_max, TIME_STEPS, pow_2_n, &Fotry, &Ktry, ts, mags, sigs, data_sets, m0, data_types);											// All threads call the chi^2 function so that each thread can calculate its associated part of the chi^2 value for when it is summed together
		__syncthreads();
...
//																																																																																				// NR routine 'amoeba', read "Numerical Recipes in C 2nd Ed.", and the NR exercise book [C] 2nd Ed. 
		...
		double *u0, double *phi, double *b, double *map_limit, int *num_points, float *dim_min, float *dim_max, int *TIME_STEPS, int *pow_2_n, double *Fo, double *K, double *ts, float *mags, float *sigs, int *data_sets,  double *m0, int *data_types)
	{
		...
		double rtol,sum,swap,ysave,ytry, Fotry, Ktry;
		...
		for (;;)
		{
			...
			ytry=amotry(p,y,psum,ihi,-1.0, u0, phi, b, map_limit, num_points, dim_min, dim_max, TIME_STEPS, pow_2_n, Fo, K, ts, mags, sigs, data_sets, m0, data_types);						// All threads call the chi^2 function so that each thread can calculate its associated part of the chi^2 value for when it is summed together
			
			__syncthreads();
			
			if (ytry <= y[ilo])
				ytry=amotry(p,y,psum,ihi,2.0, u0, phi, b, map_limit, num_points, dim_min, dim_max, TIME_STEPS, pow_2_n, Fo, K, ts, mags, sigs, data_sets, m0, data_types);						// All threads call the chi^2 function so that each thread can calculate its associated part of the chi^2 value for when it is summed together
			else if (ytry >= y[inhi])
			{
				ysave=y[ihi];
				ytry=amotry(p,y,psum,ihi,0.5, u0, phi, b, map_limit, num_points, dim_min, dim_max, TIME_STEPS, pow_2_n, Fo, K, ts, mags, sigs, data_sets, m0, data_types);						// All threads call the chi^2 function so that each thread can calculate its associated part of the chi^2 value for when it is summed together
				...
				if (ytry >= ysave)
				{
					for (i=1;i<=mpts;i++)
					{
						if (i != ilo)
						{
							...
							__syncthreads();
							ytry=chi2_controller(psum, u0, phi, b, map_limit, num_points, dim_min, dim_max, TIME_STEPS, pow_2_n, &Fotry, &Ktry, ts, mags, sigs, data_sets, m0, data_types);							// All threads call the chi^2 function so that each thread can calculate its associated part of the chi^2 value for when it is summed together
							__syncthreads();
...
//////////////////////////////////////////////////////////////	Parameter serach for u0 & phi																																																	// Function to set initial block id values for u0 and phi, then call the NR routines to run a simplex downhill procedure to solve for t0 and tE
	...
	__global__ void nested_search(double map_limit, double b, double lu0, double du0, double lphi, double dphi, double est_t0, double est_tE, int num_points, float dim_min, float dim_max, int *TIME_STEPS, double *m0, double *ts, float *mags, float *sigs, int data_sets, int *data_types, double *Fo_arr, double *K_arr, double *chi2_arr, double *t0_arr, double *tE_arr)
	{
		__shared__ double p[(NDIM*MP)+1], x[NDIM+1], y[MP+1], Fo[MP+1], K[MP+1];
		double u0, phi, ytry, Fotry, Ktry;
		int i, n, pow_2_n[MAX_DATA_SITES], nfunc;
		...
		{
			x[1] = p[(0*NDIM)+1] = est_t0 - 1;																																																													// Storing a vertice of the simplex
			x[2] = p[(0*NDIM)+1+1] = est_tE - 1;																																																												// Storing a vertice of the simplex
		}
		__syncthreads();
		ytry = chi2_controller(x, &u0, &phi, &b, &map_limit, &num_points, &dim_min, &dim_max, TIME_STEPS, pow_2_n, &Fotry, &Ktry, ts, mags, sigs, &data_sets, m0, data_types);				// All threads call the chi^2 function so that each thread can calculate its associated part of the chi^2 value for when it is summed together
		...
		if (threadIdx.x == 0)
		{
			...
			K[2] = Ktry;
			x[1] = p[(2*NDIM)+1] = est_t0 - 1;																																																													// Storing a vertice of the simplex
			x[2] = p[(2*NDIM)+1+1] = est_tE + 1;																																																												// Storing a vertice of the simplex
		}
		__syncthreads();
		ytry = chi2_controller(x, &u0, &phi, &b, &map_limit, &num_points, &dim_min, &dim_max, TIME_STEPS, pow_2_n, &Fotry, &Ktry, ts, mags, sigs, &data_sets, m0, data_types);				// All threads call the chi^2 function so that each thread can calculate its associated part of the chi^2 value for when it is summed together
		...
		amoeba(p,y,FTOL,&nfunc, &u0, &phi, &b, &map_limit, &num_points, &dim_min, &dim_max, TIME_STEPS, pow_2_n, Fo, K, ts, mags, sigs, &data_sets, m0, data_types);				// Call of NR routine to perform the simplex downhill
...
//////////////////////////////////////////////////////////////	Single chi^2 calculation
	...
	__global__ void single_chi2_calc(double t0, double tE, double u0, double phi, double b, double map_limit, int num_points, float dim_min, float dim_max, int *TIME_STEPS, double *m0, double *ts, float *mags, float *sigs, int data_sets, int *data_types, double *Fo, double *K, double *chi2)
	{
	...
	if file_id[25:28] == 'MOA':	
		...
		for line in obs_data:
			if TIME_STEPS + 1 > start_point:																																																										# If line number is bigger then when to start counting
				partial_ts[Data_row_count]		=	np.float64(line[0])-np.float64(2450000)																																								# Store the JD - 2450000
				partial_mags[Data_row_count]	=	np.float32(line[1])																																																		# Store the magnification
				partial_sigs[Data_row_count]	=	( (np.float32(line[2])*np.float32(line[2])) + (0.01*0.01) )**0.5																																																		# Store the error
				Data_row_count += 1
				if Data_row_count == DATA_LIMIT:																																																								# Ensure the loop does not store more data then the maximum limit
					break
			TIME_STEPS += 1																																																																	# Count the line numbers
	
	return Data_row_count, m0, partial_ts, partial_mags, partial_sigs
...
def chi2_calc(p):
	...
	if d < 0 or q < 0 or rho < 0 or u0 < 0:
		return -9999999
	...
	for line in global_vars:
		...
		DATA_LIMIT			=	np.float64(line[5])																																																														# DATA_LIMIT
	...
	m0				=	np.zeros([MAX_DATA_SITES],np.float64)

	data_sets = np.int32(0)
	...
	for infile in listing:
		if infile == 'associated' or infile == 'output' or infile == 'WIP':
			...
			if data_sets == 0:
				if infile[0:3] == 'MOA':
					DATA_ROWS[data_sets],m0[data_sets],ts[0:DATA_LIMIT],mags[0:DATA_LIMIT],sigs[0:DATA_LIMIT] = read_data(file_id+infile,DATA_LIMIT)																														# Read in the raw MOA data file
					data_types[data_sets] = 0
				elif infile[0:3] == 'OGL':
					DATA_ROWS[data_sets],m0[data_sets],ts[0:DATA_LIMIT],mags[0:DATA_LIMIT],sigs[0:DATA_LIMIT] = read_data(file_id+infile,DATA_LIMIT)																													# Read in the raw OGLE data file
					data_types[data_sets] = 1
				data_sets += 1
			else:
				if infile[0:3] == 'MOA':
					DATA_ROWS[data_sets],m0[data_sets],ts[DATA_ROWS[data_sets-1]:DATA_ROWS[data_sets-1]+DATA_LIMIT],mags[DATA_ROWS[data_sets-1]:DATA_ROWS[data_sets-1]+DATA_LIMIT],sigs[DATA_ROWS[data_sets-1]:DATA_ROWS[data_sets-1]+DATA_LIMIT] = read_data(file_id+infile,DATA_LIMIT)			# Read in the raw MOA data file
					data_types[data_sets] = 0
				elif infile[0:3] == 'OGL':
					DATA_ROWS[data_sets],m0[data_sets],ts[DATA_ROWS[data_sets-1]:DATA_ROWS[data_sets-1]+DATA_LIMIT],mags[DATA_ROWS[data_sets-1]:DATA_ROWS[data_sets-1]+DATA_LIMIT],sigs[DATA_ROWS[data_sets-1]:DATA_ROWS[data_sets-1]+DATA_LIMIT] = read_data(file_id+infile,DATA_LIMIT)			# Read in the raw OGLE data file
					data_types[data_sets] = 1
				data_sets += 1
	...
	map_limits = (2.0/3.0)*(math.atan(2*d-7.2))+2.46
	...
	A = np.zeros((num_points,num_points),np.float64)																																																												# Set up an empty array which will be the mag map from the CUDA function
	...
	size = math.floor(math.ceil((2*rho)/dx)/2) * 2 + 1																																																														# calculate the size that the kernel needs to be
	...
	gridshape_chi2 = (1,1)
	single_chi2_calc(np.float64(t0), np.float64(tE), np.float64(u0), np.float64(phi), np.float64(b), np.float64(map_limits), np.int32(num_points), np.float32(x0), np.float32(x0+dx*num_points-dx), drv.In(DATA_ROWS), drv.In(m0), drv.In(ts), drv.In(mags), drv.In(sigs), np.int32(data_sets), drv.In(data_types), drv.Out(Fo), drv.Out(K), drv.Out(chi2), texrefs=[texref], block=blockshape_chi2, grid=gridshape_chi2)
	
	#ln_likelihood = -chi2[0]/2
	ln_likelihood = chi2[0]
